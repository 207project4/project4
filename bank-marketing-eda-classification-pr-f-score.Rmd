...
bank_data %>% 
  ggplot() +
  geom_mosaic(aes(x = product(pdays_dummy), fill = y)) +
  mosaic_theme +
  xlab("Previous") +
  ylab(NULL)
...
rf = ranger(y ~ .,
            ...
            write.forest = T,
            probability = T)
rf_simple = ranger( y ~ . - job - marital - education - previous - euribor3m - cons.conf.idx - campaign,
            data = bank_train,
            num.trees = 1000,
            importance = "impurity",
            splitrule = ranger_tune$bestTune$splitrule,
...
fun_imp_ggplot_split(rf_simple)
```
...
rf_train_score_simple = predict(rf_simple,
                         data = bank_train)$predictions[, 2]

rf_test_score_simple = predict(rf_simple,
                        data = bank_test)$predictions[, 2]
```
...
#simple
measure_train_simple = fun_gg_cutoff(rf_train_score_simple, bank_train$y, 
                              "acc", "f")
measure_train_simple +
  geom_vline(xintercept = c(0.3, 0.5), 
             linetype = "dashed")
```
...
rf_train_cut = 0.3
rf_train_class_simple = fun_cut_predict(rf_train_score_simple, rf_train_cut)
# matrix
rf_train_confm_simple = confusionMatrix(rf_train_class_simple, bank_train$y, 
                                 positive = "1",
                                 mode = "everything")
rf_train_confm_simple
```
...
measure_test_simple = fun_gg_cutoff(rf_test_score_simple, bank_test$y, 
                             "acc", "f")
measure_test_simple +
  geom_vline(xintercept = c(rf_train_cut, 0.5), 
             linetype = "dashed")
...
#simple
rf_test_class_simple = fun_cut_predict(rf_test_score_simple, rf_train_cut)
# matrix
rf_test_confm_simple = confusionMatrix(rf_test_class_simple, bank_test$y, 
                                positive = "1",
                                mode = "everything")
rf_test_confm_simple
```
...
score_train = data.frame("logistic complex" = logistic_train_score,
                        ...
                        "random forest simple" = rf_train_score_simple,
...
score_test = data.frame("logistic complex" = logistic_test_score,
                        ...
                        "random forest simple" = rf_test_score_simple,
